...
class DummyDataset(data.Dataset):
    def __init__(self, opts, num_images=10):
        ...
        for i in range(self.num_domains):
            ...
            dir_iter = os.scandir(img_dir)
            filelist = []
            for n in range(num_images):
                filelist.append(next(dir_iter).name)
            self.images[i] = [os.path.join(img_dir, x) for x in filelist]
