...
class MD_multi(nn.Module):
    def __init__(self, opts):
        ...
        self.disContent_opt = torch.optim.Adam(self.disContent.parameters(), lr=lr/2.5, betas=(0.5, 0.999), weight_decay=0.0001)
...
#             print(self.mu)
#             print(self.logvar)
            std = torch.exp(self.logvar.mul(0.5)) # <-------- inf values are obtained in this operations
...
#         print('--nan values generated ---------------------')
        ...
        if self.concat:
            ...
            eps_recon = self.get_z_random(std_recon.size(0), std_recon.size(1), 'gauss').to(device)
...
#         self.gen_opt.step()
    ...
    def backward_EG(self):
        ...
        # KL loss - z_a
        if self.concat:
            kl_element = torch.add((- torch.add(self.mu.pow(2), self.logvar.exp())) + 1 , self.logvar)
